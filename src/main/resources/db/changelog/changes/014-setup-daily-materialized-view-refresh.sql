...
BEGIN
    ...
    FROM cron.job 
    WHERE command LIKE '%refresh_active_users_mv%';
    ...
    SELECT cron.schedule(
        'daily-mv-refresh',
        '0 0 * * *',
        'SELECT refresh_active_users_mv();'
    ) INTO job_id;
...
-- Add comment
COMMENT ON FUNCTION refresh_active_users_mv() IS 'Refresh active users materialized view with error handling and logging';
